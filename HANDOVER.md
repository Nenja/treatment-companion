...
- **`admin-collapsible-nav-1` — admin floating side-nav + collapsible sections, login language-switcher fix, REDCap sync result fixes (cumulative; deploy this ONE zip; no migration).** Folds in everything below incl. the deploy-hardening workflow.
