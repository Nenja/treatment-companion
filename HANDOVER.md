...
>
> _Last updated for batch: `e2e-autorun-1`. Promoted the E2E smoke workflow (`.github/workflows/e2e.yml`) from manual-only to **auto-running**: kept `workflow_dispatch`, added a **daily `schedule`** (06:00 UTC synthetic monitoring) and a **`deployment_status`** trigger (runs after a SUCCESSFUL **Production** Vercel deploy), added a `concurrency` group, and kept it **off push/PR** (a browser flake must never red-gate a commit — CI's verify job remains the push gate). Workflow-file only — no app, schema, or dependency change. Prereqs unchanged (repo Variable `E2E_BASE_URL` + Secrets `E2E_PATIENT_EMAIL`/`E2E_PATIENT_PASSWORD`). Schedule + deployment_status only take effect once the file is on `main`. Data note: each auto run completes a weekly check-in as the test patient (self-skips once the prompt is used) — use a dedicated e2e test account. YAML validated. _Earlier still-pending deploy items (independent of this): the `next16-upgrade-1` proxy rename (add `proxy.ts`, delete `middleware.ts`) and running `0108` in the SQL editor — see those §7 entries._ See §7._
