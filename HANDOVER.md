...
- **`deploy-hardening-2` — inject build-time public env from CI secrets (current; no app-code change).** Making `NEXT_PUBLIC_*` non-Sensitive should have let `vercel pull` deliver them, but the 500s persisted, so the workflow no longer depends on `vercel pull` for them: `deploy.yml` now writes `NEXT_PUBLIC_SUPABASE_URL`/`ANON_KEY` (from **GitHub repo secrets**) into `.vercel/.env.production.local` before `vercel build`, removing the sensitivity/scope dependency entirely. Keeps the fail-fast (missing secrets) and the post-build `supabase.co`-inlined check. **One-time setup: add repo secrets `NEXT_PUBLIC_SUPABASE_URL` + `NEXT_PUBLIC_SUPABASE_ANON_KEY` (production values; public, safe in CI).** Doc: `docs/STAGING-AND-CI-GATE.md` → Troubleshooting + "injected from CI secrets". NOTE: this only takes effect once the updated `deploy.yml` is actually in the repo — if a 500 persisted earlier it may be because a direct GitHub README edit ran the *old* workflow.
