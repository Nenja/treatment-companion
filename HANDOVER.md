...
>
> _Last updated for batch: `suggestion-approve-redesign-1` (**NEW migration 0107**). Redesigned the clinician **approve-suggestion** page around how review actually happens (at the visit): the five actions collapse to two — **Approve as a goal** and **Set aside**. Approve now has a mode toggle — **As a new goal** (the existing NRS/GAS authoring flow) or **Add to an existing goal**, which folds the suggestion into one of the patient's current goals (records status + which goal; creates no new goal and leaves the target untouched). "Discuss at next visit" and standalone "Combine" are gone — combine moved into Approve, and deferral is implicit (an un-actioned suggestion stays pending and resurfaces next visit). NEW migration **0107** (`combined_into_goal_id` column + clinician-gated `combine_suggestion_into_goal` RPC). Build 110/110, tsc clean, parity holds (1663 keys), 41/41 tests pass. **0107 verified on throwaway Postgres** (focused harness exercises the RPC + all four guards; full 0001→0107 CI chain replays with 0 failures). **Run 0107 in the Supabase SQL editor.** sv/nb strings first-pass — flag for native review. See §7._
