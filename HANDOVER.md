...
>
> _Last updated for batch: `consent-consolidation-1` (**CUMULATIVE — also carries `suggestion-approve-redesign-1` + its migration 0107**; no new migration of its own). Consolidated patient consent into ONE place: the toolbar's "Video" action is renamed **Consent** (new shield-check icon), and the Consent panel now shows all three dimensions — **clinical recording / educational use / research participation** — each as a status pill + grant/withdraw. The research + educational consent rows were **removed from the Background card**, so consent lives only in the Consent panel. New top-level `clinicalConsent` i18n namespace; panel title/intro + action labels relabelled. No schema change. Build 110/110, tsc clean, parity holds (1668 keys), 41/41 tests pass. **Still run migration 0107** (from the carried suggestion batch) in Supabase. sv/nb strings first-pass. See §7._
