...
>
> _Last updated for build tag: `simplify-cockpit-89` (**no migration**). Fixes the clinician **history** page 400: the history hook selected `treatment_cycle.length_weeks`, a column that was DROPPED in migration 0010 (with `review_date`) and never re-added — so the query failed on any current DB. `length_weeks` was selected/stored but never rendered; removed it from the select, the mapping, and the type. History now loads. See §7. (Also flags a related dictionary bug — see chat.)_
