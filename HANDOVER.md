...
>
> _Last updated for batch: `adjustment-loop-1` (**CUMULATIVE; carries the sv/nb translation, language picker, EHR rework, auto-apply locale, pg_cron 0104**). Built the therapist adjustment-request **status loop (option A — clinician-side only)**: NEW migration **0105** (adjustment_status open->addressed/dismissed + clinician-only `resolve_adjustment_request` RPC), with Add/Dismiss buttons on both clinician surfaces; resolving drops the request off the open list, therapist sees nothing. Build 110/110, tsc clean, parity holds, **0105 verified on throwaway Postgres (all guards + idempotency)**. Also delivered (separate files, not in the web zip): the **finalised REDCap data dictionary** + reconciliation report. **Next: the pseudonymised CSV export — gated on the record_id/study-pseudonym decision.** See §7._
