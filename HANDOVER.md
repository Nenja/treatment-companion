...
>
> _Last updated for build tag: `simplify-cockpit-86` (**migration 0099 — DEV SEED ONLY**). Enriches the demo patients (test1–test6) with everything the clinician **history** page reads but the base seed omitted: diagnosis (etiology) + medication, per-goal **clinician video ratings** and **physiotherapist assessments/ratings**, check-in **side effects**, and per-cycle physician→therapist **notes**. New `dev_seed_history_extras()` is wired into `dev_reseed_all()`. Run 0099, then reseed. Method-D verified + idempotent. (Depends on 0097 for the `anoxic` etiology value.) See §7._
