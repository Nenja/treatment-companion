...
>
> _Last updated for batch: `reminder-cron-1` (**CUMULATIVE; carries the sv/nb translation + language picker + EHR export rework + migration 0103, safe to re-run**). Two things: (1) the login redirect now **auto-applies the user's saved language** on sign-in (preferred_locale wins over the login page's locale); (2) NEW migration **0104** schedules the `send-checkin-notifications` Edge Function daily via pg_cron + pg_net, secrets read from Vault by name (no secret in the repo). Build 110/110, tsc clean, parity holds. **The Edge Function deploy + the two Vault secrets + running 0104 are manual Supabase-dashboard/SQL steps — see BUILD.txt.** See §7._
