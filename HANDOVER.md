...
>
> _Last updated for batch: `consolidate-charts-1` (**CUMULATIVE; carries the adjustment loop 0105, sv/nb translation, language picker, EHR rework, auto-apply locale, pg_cron 0104**). Built the **pseudonymised REDCap CSV export**: NEW migration **0106** (a stable `study_code` pseudonym + a clinician-gated `export_research_dataset()` RPC that assembles every research-consented patient across all cycles/instruments as JSONB), a client (`lib/redcapExport.ts`) that flattens + codes it into a REDCap import CSV, and a Download button on the clinician observations page. Build 110/110, tsc clean, parity holds. **0106 verified on throwaway Postgres** (shape, consent scoping, per-cycle goal_index linking, non-clinician gate, idempotency); client column-mapping verified consistent. **record_id = approach A (app-assigned TC-NNNN) — DPO to bless or swap. The CSV must be test-imported into a real REDCap project.** Also retired the orphaned `/physio/progress` route to a locale-preserving redirect (the form lives inline on `/physio/patient`); build 110/110. See §7._
