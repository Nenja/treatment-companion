...
>
> _Last updated for batch: `deps-secfix-1`. **Two things:** (1) **next-intl 4.4.0 → 4.13.0** to fix the open-redirect advisory **GHSA-8f24-v5vv-gm5j** (CVSS 5.3; patched in 4.9.1). The `e2e-autorun-1`/earlier `4.4.0` pin had been chosen for lockfile portability, but 4.4.0 carries this CVE — security wins, so it's reverted to the patched 4.13.0 (which re-introduces the native `@swc/core` dep). The lock was **clean-regenerated and `npm ci`-verified on the exact Node-20 npm versions (10.2.4 + 10.8.2)** that failed before, so a correctly-committed lock is solid — **commit `package.json` + `package-lock.json` together.** (2) Added **`.github/dependabot.yml`** (weekly npm + github-actions update PRs) so future CVEs like this are caught and fixed automatically. Build 109/109, tsc clean, parity 1668, 41/41 tests. Remaining audit items: `@supabase/auth-js` (Dependabot will propose the `@supabase/supabase-js` bump) and a transitive `postcss` under `next` (clears when next bumps it). Enable the Dependabot **alerts** + **security updates** toggles in repo settings. See §7._
