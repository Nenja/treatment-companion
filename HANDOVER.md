...
>
> _Last updated for build tag: `simplify-cockpit-71` (no new migration; needs 0095). The 1080 therapist cockpit layout: goals are now **horizontal bands** (trend chart left / rating right, hairline divider) instead of stacked cards; the physician note + patient comments sit as a **top context row** rather than a left rail (rail retired); the page widened to 1080. `GoalProgressView` gained a `bare` prop (no outer card) so the chart embeds in the band. Also fixed a cockpit-70 bug where three labels referenced undefined message keys. See §7._
