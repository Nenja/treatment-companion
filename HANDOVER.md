...
>
> _Last updated for build tag: `simplify-cockpit-85` (**no migration**). Three clinician-page refinements: the therapist-suggestion review now has a single **Mark considered** action (the redundant Dismiss, which did the same thing, is gone — all four spots); the physician→therapist **handoff note** drops the "Did you change the treatment?" toggle (just the written note now); and the note's contradictory **"Therapist only" badge is removed** — that note is patient-readable by design (migration 0096), which the hint already states correctly. See §7._
