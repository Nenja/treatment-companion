...
>
> _Last updated for batch: `secdef-harden-1`. **CUMULATIVE zip** — also carries the still-uncommitted `next16-upgrade-1` (Next 16.2.7 + next-intl 4.4.0 + Sentry 10.58.0 + `proxy.ts`; **delete `middleware.ts`**). This batch: SECURITY DEFINER audit (grounded in a Method-D replay + catalog queries) + migration **`0108_harden_secdef_functions.sql`**. Reconciled the count (the assessment's "134" was wrong — it counted Supabase system schemas; the app has **83** SECURITY DEFINER fns in `public`). 0108 (A) pins `search_path = public` on the **17** functions that still had a mutable path (66 already did), and (B) revokes EXECUTE on the **10 destructive `dev_seed_*` functions** from PUBLIC/anon/authenticated and re-grants only `service_role` (the app's `/api/dev/scenario` route calls them via service-role, so it keeps working). Caller-gate review: the 22 cross-user (`p_patient_id`) functions are **all** gated (`clinician_can_access_patient` = active, <1h session). **FORCE RLS deliberately NOT applied** — 16 `TO authenticated` policies would silent-deny the postgres-owned definer context, and it's unverifiable in the harness (there postgres has BYPASSRLS); see `docs/audits/security-definer-audit-2026-06.md` §F3. Verified by from-scratch CI replay incl. 0108. Two follow-ups left (F2 broad grant-tightening; F3 staged FORCE RLS). To deploy: drop the Next 16 files + delete `middleware.ts`, then run `0108` in the SQL editor. **(next-intl pinned to 4.4.0, not 4.13.0, so the lockfile has no native deps and `npm ci` can't drift — this fixes the CI 'Missing @swc/helpers@0.5.23' error.)** See §7._
