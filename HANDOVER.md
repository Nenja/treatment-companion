...
>
> _Last updated for batch: `e2e-smoke-1` (**CUMULATIVE — also carries `consent-consolidation-1` + `suggestion-approve-redesign-1` + migration 0107**; no app-code or schema change of its own). Added a Playwright **end-to-end smoke** scaffold (`playwright.config.ts`, `e2e/smoke.spec.ts`, `e2e/README.md`, a manual-only `e2e.yml` workflow) covering login + a patient completing a weekly check-in; `e2e` is excluded from the app tsconfig so `npm run verify` is unaffected. **Honest:** authored without being run (no browser here) — the two unauthenticated tests are robust; the two authenticated tests are env-gated (skip without creds) and may need a first-run selector tweak. The e2e files DO type-check clean against @playwright/test. This **completes the audit roadmap** (test harness · privacy/DPIA · native push · sv/nb reminders · E2E). Build 110/110, tsc clean, 41/41 tests, parity 1668. **Still run migration 0107** in Supabase. See §7._
