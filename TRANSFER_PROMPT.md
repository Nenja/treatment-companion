...
- **Latest build:** `checkin-e2e-fix-1`. Fixed the Playwright check-in smoke test (it timed out on a disabled Continue — a test race against the loading skeleton, not an app bug); it now waits for the wizard to render, confirms each rating registered before advancing, and SKIPS gracefully when there's no pending prompt so it's re-runnable. Carries the `auth-redirect-guard-1` app fix (global signed-out→/login guard in `lib/supabase/auth.tsx`) so one drop deploys both. No schema change. App build 110/110, tsc clean, 41/41 tests; e2e spec type-checks. After deploy expect 4 passed (or 3 passed + 1 skipped if the test patient's check-in was already consumed).
