...
- **Latest build:** `next16-upgrade-1`. **Next.js 15.1.9 → 16.2.7** (closes unpatched CVEs incl. a CVSS-10 RCE), with forced coupled bumps **next-intl 3.26.5 → 4.13.0** and **@sentry/nextjs 8.42.0 → 10.58.0**. App was already async-API-ready, so the only code edit was renaming **`middleware.ts` → `proxy.ts`** (Next 16 convention; **delete the old `middleware.ts`**). Build runs on Turbopack, 109/109, tsc clean, parity 1668, 41/41 tests, e2e spec type-clean; high-severity audit advisories resolved. Ship `package.json` + `package-lock.json` + `proxy.ts`, delete `middleware.ts`. Runtime not verifiable from sandbox — after deploy re-run the E2E smoke + click through all four locales + check session persistence.
