...
- **Latest build:** `e2e-autorun-1` — promoted `.github/workflows/e2e.yml` from manual-only to auto-running: kept `workflow_dispatch`, added a daily `schedule` (06:00 UTC) and a `deployment_status` trigger (after a successful **Production** Vercel deploy), a `concurrency` group, and kept it **off push/PR** (CI's verify job stays the push gate). Workflow-file only — no app/schema/dependency change. Prereqs: repo Variable `E2E_BASE_URL` + Secrets `E2E_PATIENT_EMAIL`/`E2E_PATIENT_PASSWORD`; schedule + deployment_status only take effect from `main`; each auto run consumes the test patient's weekly prompt (self-skips after). Deploy: drop the workflow file on `main`. _Independent earlier still-pending items: `next16-upgrade-1` proxy rename (add `proxy.ts`, delete `middleware.ts`) and running `0108` in the SQL editor — see HANDOVER §7._ Previously: secdef-harden-1 (migration 0108 + SECURITY DEFINER audit) and next16-upgrade-1 (Next 16.2.7 + next-intl 4.4.0 + Sentry 10.58.0).
