...
- **Latest build:** `checkin-e2e-fix-2`. Test-only follow-up: the check-in smoke now drives off the button text ("Continue" vs final "Send my check-in"), submits on the last step and breaks before the button is replaced by the thanks screen — fixing the `toBeEnabled … element(s) not found` failure. The signed-out→/login guard (`auth-redirect-guard-1`) is already deployed. No app/schema change. Re-run the smoke for **4 passed** (or **3 passed + 1 skipped** if test1's prompt was already consumed).
