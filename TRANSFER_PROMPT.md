...
- **Then `hooks-refactor-1` (in the repo; NO SQL to run):** fixed that violation. Split `app/[locale]/clinician/treatment/page.tsx` into a shell (fetch + guards) and a child `TreatmentRecordLoaded` holding all loaded-data hooks, so hooks run unconditionally - latent crash gone. Scoped ESLint warning removed; `rules-of-hooks` is now a hard error everywhere. tsc/lint/build green; no migration; behaviour preserved (wholesale hook move). **QA the treatment screen** (record/edit/new-cycle/save/copy-from-previous/rail-scroll-highlight/total auto-fill). Detail in `HANDOVER.md` §7.
