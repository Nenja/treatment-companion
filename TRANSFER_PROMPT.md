...
- **Latest build:** `deps-secfix-1` — (1) **next-intl 4.4.0 → 4.13.0** to fix open-redirect **GHSA-8f24-v5vv-gm5j** (CVSS 5.3; patched 4.9.1). The earlier 4.4.0 pin (chosen for lockfile portability) carries this CVE — security wins, so reverted; `@swc/core` returns but the lock was clean-regenerated and `npm ci`-verified on Node-20 npm (10.2.4 + 10.8.2). **Commit package.json + package-lock.json together.** (2) Added **`.github/dependabot.yml`** (weekly npm + github-actions PRs; each updates both files + is CI-checked) — enable the Dependabot **alerts** + **security updates** toggles in settings. Build 109/109, tsc clean, parity 1668, 41/41. Remaining audit: `@supabase/auth-js` (Dependabot will bump supabase-js) + transitive `postcss` under next. _Independent earlier still-pending: next16 proxy/middleware swap, run `0108` in SQL editor, commit the `e2e-autorun-1` workflow._ Prior: e2e-autorun-1 (auto E2E), secdef-harden-1 (0108 + audit), next16-upgrade-1 (Next 16.2.7 + Sentry 10.58.0).
