...
- **Latest build:** `language-picker-1` (**CUMULATIVE zip**; carries the completed sv/nb translation + migration 0103, safe to re-run) — four-language picker: `EN·DA·SV·NB` segmented control on login, endonym cards in profile/settings (applies live, persists `preferred_locale`). Build 110/110, tsc clean.
- **Just shipped:** `language-picker-1` — the language picker (login + settings). New `LanguageSelect` component + `useSetPreferredLocale` hook + one heading key in 4 langs. Known follow-up: preferred_locale isn't auto-applied on sign-in yet.
- **In progress / next:** the **EHR export rework** — (1) rewrite the text builder (`lib/ehrExport.ts`) to the agreed portable format: TREATMENT + GOALS only (no name, no patient comments), NRS goals as `baseline → target` + best/end + wearing-off with scale direction, GAS goals in WORDS (much worse…much better) + the achieved anchor description + best/end + wearing-off; update the `ehrExport` strings in all 4 langs; extend the call site to pass each goal's baseline/target/direction + anchor text. (2) Add a **per-goal GAS/NRS chart PNG export** button to `components/clinician/ExportModal.tsx` — a dedicated print-styled chart→PNG renderer (white bg, fixed width), one image per goal, mirroring how `FaceMap.tsx` builds its export image. Mockups already approved. Build is **110/110**.
