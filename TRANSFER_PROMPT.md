...
- **Latest build:** `secdef-harden-1` — **CUMULATIVE zip**, also carries the uncommitted `next16-upgrade-1` (Next 16.2.7 + next-intl 4.4.0 + Sentry 10.58.0 + `proxy.ts`; **delete `middleware.ts`**). This batch: a SECURITY DEFINER audit (Method-D replay + catalog queries) + migration `0108_harden_secdef_functions.sql` — pins `search_path = public` on the 17 functions that lacked it (66 already had it), and revokes EXECUTE on the 10 destructive `dev_seed_*` functions from anon/authenticated/PUBLIC, granting only `service_role` (the `/api/dev/scenario` route uses service-role, so unaffected). Count reconciled: 83 app SECURITY DEFINER fns, not the assessment's 134. The 22 `p_patient_id` functions are all gated (`clinician_can_access_patient` = active <1h session). FORCE RLS deliberately NOT applied (16 `TO authenticated` policies → definer-context silent-deny; unverifiable in harness) — see `docs/audits/security-definer-audit-2026-06.md` §F3. Verified by from-scratch CI replay incl. 0108. Apply: run `0108` in the Supabase SQL editor; deploy the Next 16 files + delete `middleware.ts`. Follow-ups: audit F2 (grant-tightening), F3 (staged FORCE RLS).
