...
- **Latest delivery — `admin-overview-search-1` (cumulative; deploy this ONE zip and discard the intermediate ones).** It folds in everything from the 2026-06-16 session:
  - **`0111_fix_export_guidance.sql` — RUN in staging (already run in production).** The research-export RPC read `m.guidance`, but `0009` had moved that column to the session; fixed to `s.guidance`. Method-D verified (reproduced the live error, confirmed the fix). Standalone SQL in outputs.
  - **REDCap sync automation is live and proven** — admin button + weekly cron (`/api/cron/redcap-sync`, `CRON_SECRET`-gated). A real run did **187 rows / 2 patients** against the production *test* patients pointed at a REDCap **TEST** project. Env vars (Production, server-only): `REDCAP_API_URL`, `REDCAP_API_TOKEN`, `CRON_SECRET`.
  - **Research export relocated** off the unlinked wearable page onto the **admin page**. Admin is reachable from the clinician home, the account menu, and now an **Admin entry in the patient Tools rail** (gear icon, admin-only).
  - **Admin page restructured:** overview band (at-a-glance account counts + jump-links) + two grouped domains — *Accounts & access* (Accounts · Create · Active access) and *Research data* (Research export · Studies · Consent pending-deletion). **Free-text search added to the Studies section** (studies by key/name/desc; participants by name/REDCap id) on top of the existing filters. The accounts list already had name/email search.
  - **Dev scenario launcher RETIRED** (`/dev/scenarios`, `/api/dev/scenario`, `lib/dev/scenarios.ts`) — it signed you out then failed to sign back in (the "bounces to login even when logged in" bug). `ENABLE_DEV_TOOLS` / `NEXT_PUBLIC_ENABLE_DEV_TOOLS` are now dead env vars — delete from Vercel. `dev_reseed_all()` SQL is untouched.
...
- **Before REDCap pilot go-live:** clear the REDCap **test** project rows; point `REDCAP_API_URL`/`TOKEN` at the **real study** project; set `CRON_SECRET` if enabling the weekly cron.
