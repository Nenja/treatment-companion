-- 0066_dev_seed_functions.sql
-- ---------------------------------------------------------------------------
-- DEV-ONLY. Wraps the six demo-patient seed blocks (verbatim) from
-- demo_seed_test_patients.sql into callable functions so the dev Scenarios
-- launcher can reset the demo data with one click instead of pasting the
-- seed into the SQL editor. Same prerequisites as the seed file: the six
-- test accounts and at least one clinician must already exist.
--
-- dev_reseed_all() runs every block in order (identical to running the seed
-- file). Each block is its own SECURITY DEFINER function with its original
-- body unchanged.
-- ---------------------------------------------------------------------------

create or replace function dev_seed_b1() returns void
  language plpgsql security definer set search_path = public as $devseed$
declare
  v_profile_id uuid;
  v_patient_id uuid;
  v_clinician_id uuid;
...
create or replace function dev_seed_b2() returns void
  language plpgsql security definer set search_path = public as $devseed$
declare
  v_profile_id uuid;
  v_patient_id uuid;
...
create or replace function dev_seed_b3() returns void
  language plpgsql security definer set search_path = public as $devseed$
declare
  v_profile_id uuid;
  v_patient_id uuid;
...
create or replace function dev_seed_b4() returns void
  language plpgsql security definer set search_path = public as $devseed$
declare
  v_profile_id uuid;
  v_patient_id uuid;
...
create or replace function dev_seed_b5() returns void
  language plpgsql security definer set search_path = public as $devseed$
declare
  v_profile_id uuid;
  v_patient_id uuid;
...
create or replace function dev_seed_b6() returns void
  language plpgsql security definer set search_path = public as $devseed$
declare
  v_profile_id uuid;
  v_patient_id uuid;
...
create or replace function dev_seed_b7() returns void
  language plpgsql security definer set search_path = public as $devseed$
declare
  v_rec record;
  v_patient_id uuid;
...
create or replace function dev_seed_b8() returns void
  language plpgsql security definer set search_path = public as $devseed$
declare
  v_patient_id uuid;
  v_cycle_id   uuid;
