...
const COLUMNS: string[] = [
  ...
  // treatment
  'tx_cycle_index', 'tx_date', 'tx_drug_product', 'tx_total_units', 'tx_dilution', 'tx_notes',
  // muscle
  ...
  'ci_spasm_freq', 'ci_daily_care', ...SIDE_EFFECT_COLS, 'ci_side_effect_other', 'ci_comment',
  ...
  // physio_assessment
  'pa_cycle_index', 'pa_date', 'pa_note',
  // physio_goal_rating
  'pgr_cycle_index', 'pgr_assessment_date', 'pgr_goal_index', 'pgr_nrs_value', 'pgr_gas_value',
  'pgr_working_on', 'pgr_needs_adjustment', 'pgr_adjustment_note',
  // itb
  'itb_started_on', 'itb_ended_on', 'itb_note',
  // itb_dose_change
  'idc_started_on', 'idc_changed_on', 'idc_dose_mcg_day', 'idc_note'
];
...
function patientRows(p: any): Row[] {
  ...
  each(p.sessions, (s, i) => ({
    ...
    tx_total_units: num(s.total_units), tx_dilution: txt(s.dilution), tx_notes: txt(s.notes)
  }));
  each(p.muscles, (m, i) => ({
    ...base('muscle', i + 1),
  ...
  each(p.checkins, (w, i) => {
    const r: Row = {
      ...
      ci_side_effect_other: txt(w.other_side_effect_text), ci_comment: txt(w.comment),
  ...
  each(p.physio, (a, i) => ({
    ...
    pa_cycle_index: num(a.cycle_number), pa_date: ymd(a.assessment_date), pa_note: txt(a.note)
  }));
  each(p.physio_ratings, (r0, i) => ({
    ...
    pgr_adjustment_note: txt(r0.adjustment_note)
  }));
  each(p.itb, (it, i) => ({
    ...base('itb', i + 1),
    itb_started_on: ymd(it.started_on), itb_ended_on: ymd(it.ended_on), itb_note: txt(it.note)
  }));
  each(p.itb_doses, (d, i) => ({
    ...base('itb_dose_change', i + 1),
    idc_started_on: ymd(d.started_on), idc_changed_on: ymd(d.changed_on),
    idc_dose_mcg_day: num(d.dose_mcg_per_day), idc_note: txt(d.note)
  }));
  return rows;
}
